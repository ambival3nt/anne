<div data-theme="mytheme" class="h-full overflow-y-auto mt-fluid-l mb-fluid-m">

    <div class="flex justify-center items-center">
        <div class="join">
            <button type="button" class="join-item btn btn-sm btn-outline btn-primary opacity-80" wire:click="pageHandler('1')">«
            </button>
            <button type="button" class="join-item btn btn-sm btn-outline btn-primary opacity-80" wire:click="pageHandler('{{json_decode($logData)->current_page - 1 }}')">«
            </button>

            <button type="button" class="join-item btn btn-sm btn-outline btn-primary opacity-80">
                Page {{json_decode($logData)->current_page}} / {{json_decode($logData)->last_page}}</button>
            <button type="button" class="join-item btn btn-sm btn-outline btn-primary opacity-80" wire:click="pageHandler('{{json_decode($logData)->current_page + 1}}')">»
            </button>
            <button type="button" class="join-item btn btn-sm btn-outline btn-primary opacity-80" wire:click="pageHandler('{{json_decode($logData)->last_page}}')">»
            </button>
        </div>
    </div>
